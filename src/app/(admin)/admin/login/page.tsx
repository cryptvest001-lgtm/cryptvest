...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    setLoading(true);
    const res = await apiPostPublic("/admin/auth/login", { email, password });
    setLoading(false);
    if (!res.ok) {
    ...
    <main className="relative flex min-h-screen items-center justify-center px-4 overflow-hidden bg-radial-pulse">
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <div className="glass p-8 space-y-6">
          ...
          {error && <div className="alert-red text-sm">{error}</div>}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="input-dark w-full px-4 py-3 text-sm outline-none"
                placeholder="admin@cryptvest.com"
                required
              />
            </div>
            <div>
