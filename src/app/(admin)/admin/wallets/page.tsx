...
export default function AdminWalletsPage() {
  ...
  async function load(showRefresh = false) {
    ...
    const res = await apiGetAdmin("/admin/wallets");
    if (res.ok) {
  ...
  return (
    <main className="mx-auto max-w-6xl p-4 sm:p-6 space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
        <div className="glass p-5">
          ...
          <p
            ...
            style={{ color: "#f59e0b" }}
          >
            {loading ? "—" : `${totalBtc.toFixed(8)} BTC`}
          </p>
          <p
            className="text-xs mt-1"
            style={{ color: "rgba(226,232,240,0.45)" }}
          >
            {btc.length} address{btc.length !== 1 ? "es" : ""}
          </p>
        </div>
        <div className="glass p-5">
          <p
          ...
          <p
            ...
            style={{ color: "rgba(226,232,240,0.45)" }}
          >
            {erc20.length} address{erc20.length !== 1 ? "es" : ""}
          </p>
        </div>
      </div>
              ...
              <table className="w-full text-sm min-w-[560px]">
                ...
                <tbody className="table-dark">
                  {btc.map((w) => (
                    <tr key={w.address}>
                      <td
                        className="px-5 py-3 font-mono text-xs truncate max-w-xs"
      ...
      {!loading && btc.length === 0 && erc20.length === 0 && (
        <div
          className="glass p-8 text-center text-sm"
          style={{ color: "rgba(226,232,240,0.45)" }}
        >
          No deposit addresses generated yet. Users must request a deposit
          address first.
        </div>
      )}
    </main>
