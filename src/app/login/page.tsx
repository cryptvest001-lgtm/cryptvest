...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (params.get("registered") === "1") {
      setSuccess(
        "Account created. Check your email to verify it before signing in. In local dev, the verification link is printed in the API logs when email sending is mocked.",
      );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSuccess("");
    setLoading(true);
    const res = await apiPostPublic("/auth/login", { email, password });
    setLoading(false);
    if (!res.ok) {
