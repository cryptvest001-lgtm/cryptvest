...
export default function DashboardPage() {
  ...
  const [user, setUser] = useState<{ email: string; kycStatus: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    Promise.all([apiGetUser("/deposits/balances"), apiGetUser("/stakes/my")])
  ...
  const totalEarnings = stakes.reduce(
    (sum, s) => sum + Number(s.accruedEarnings),
    0,
  );
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
        ...
        {user && (
          <div className="flex items-center gap-3">
            ...
            {user.kycStatus !== "APPROVED" && (
              <Link href="/kyc" className="badge badge-cyan">
                Complete KYC
              </Link>
            )}
          </div>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
        <div className="glass p-5">
          ...
          {loading ? (
            <div
              className="mt-2 h-8 w-24 animate-pulse rounded-xl"
          ...
          {loading ? (
            ...
            <>
              <p
                ...
                style={{ color: "#a855f7" }}
              >
                {totals.staked.toFixed(8)}
              </p>
              <p
                className="mt-2 text-xs"
                style={{ color: "rgba(226,232,240,0.45)" }}
              >
                {activeStakes.length} active stake
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Link
          href="/deposit"
          className="group glass p-4 flex items-center gap-3 transition-all hover:-translate-y-0.5"
            ...
            {activeStakes.map((s) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  >
                    {Number(s.principal).toFixed(8)}
                  </p>
                  <p
                    className="font-mono text-xs tabular-nums"
                    style={{ color: "#22c55e" }}
                  >
                    +{Number(s.accruedEarnings).toFixed(8)}
                  </p>
                </div>
              </div>
            ))}
